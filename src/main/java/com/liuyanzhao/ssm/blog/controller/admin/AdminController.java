...
public class AdminController {
    ...
    public String loginVerify(HttpServletRequest request, HttpServletResponse response) {
        ...
        String veryfy = request.getParameter("veryfy");
        //马上删除session中的验证码
        ...
        if (user == null) {
            ...
            map.put("code", 0);
            map.put("msg", "密码错误！");
        } else if(!veryfy.equalsIgnoreCase(token) || veryfy == null){
            map.put("code", 0);
            map.put("msg", "验证码错误！");
        }
    ...
    public String logout(HttpSession session) {
        ...
        session.invalidate();//清除当前session的所有数据
    ...
    public String saveProfile(User user, HttpSession session) {
        ...
        for (int i = 0; i < comments.size(); i++) {
            commentService.updateBatchComment(user);
        }
